...
const getStringFromObject = (array) => {
  let finalArray =  '';
  const result = Object.entries(array).map(
    ([key, value]) => {
      const temp =
      `
...
export const replaceFunction = (tempString,allLabour,allNewParts,currentGst,claim,allDepreciations)=>{
  ...
  console.log("getDepreciationArray",getDepreciationArray(allNewParts,allDepreciations,claim))
  ...
  string = string?.replace(
    ...
    getNetAssessedWithLessExcessDeduction(claim,allLabour,allNewParts,currentGst)
  );
  ...
  string = string?.replace(
    ...
    finalNetAssessedAmountWithIMTAndSalvage(claim,allLabour,allNewParts,currentGst)
  );
  ...
  string = string?.replace(
    "**IDV+Rti_Amount**",
    Number(claim?.totalLoss?.Rti_Amount) +
